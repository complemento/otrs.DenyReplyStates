...
# COMPLEMENTO = DO NOT REPLY IF STATE = RESOLVIDO
#        if ( $FollowUpPossible =~ /(new ticket|reject)/i && $State{TypeName} =~ /^close/i ) {
        if ( ($FollowUpPossible =~ /(new ticket|reject)/i && ($State{TypeName} =~ /^close/i )
        || $State{Name} ~~ $ConfigObject->Get('Complemento::CustomerInterfaceImprovements::DenyReplyStates')
        ) ) {
# EO COMPLEMENTO
            my $Output = $LayoutObject->CustomerHeader( Title => 'Error' );
...
#            || $State{TypeName} !~ /^close/i
            || ($State{TypeName} !~ /^close/i && !($State{Name} ~~ $ConfigObject->Get('Complemento::CustomerInterfaceImprovements::DenyReplyStates')) )
# EO COMPLEMENTO            
        )
        )
    {
